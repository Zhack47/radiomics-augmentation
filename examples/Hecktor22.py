...
models = {"icare10" : BaggedIcareSurvival(n_estimators=10,
                                          parameters_sets=None,
                                          aggregation_method='median',
                                          n_jobs=-1),
          "icare100": BaggedIcareSurvival(n_estimators=100,
                                          parameters_sets=None,
                                          aggregation_method='median',
                                          n_jobs=-1),
...
if model_name not in models.keys():
    raise ValueError(f"Model {model_name} not found."
                     f"Please choose among {models.keys()}")

n_repeats = 10

# Opening a CSV file to store metric values
if augmented:
    csv_file = open(f"../csvs/Performance/"
                    f"Perf_Hecktor_augmented_{model_name}.csv", "w")
else:
    csv_file = open(f"../csvs/Performance/"
                    f"Perf_Hecktor_base_{model_name}.csv", "w")

csv_file.write("Feat_nb,CI_train,CI_test,cdAUC_train,cdAUC_test,ConfInt\n")
...
if augmented:
    endpoints = pd.read_csv("../csvs/hecktor2022_endpoint_training.csv")  # TODO: Maybe try using index again
    clinical = pd.read_csv("../csvs/hecktor2022_clinical_info_training.csv")  # TODO: Maybe try using index again
else:
    endpoints = pd.read_csv("../csvs/hecktor2022_endpoint_training.csv",
                            index_col="PatientID")
    clinical = pd.read_csv("../csvs/hecktor2022_clinical_info_training.csv",
                           index_col="PatientID")
...
# Maybe we should normalize the whole df
# (Or at least all samples coming from a same patient)
...
if augmented:
    ...
    # print(df_train.shape)
    df_train = pd.merge(df_train, clinical,
                        left_on="Patient Name",
                        right_on="PatientID")
    # print(df_train.shape)
    df_train = pd.merge(df_train, endpoints,
                        left_on="Patient Name",
                        right_on="PatientID")
    # print(df_train.shape)
...
kfold = StratifiedKFold(5, random_state=np.random.randint(0, 1e9),
                        shuffle=True)
...
for thr in tqdm(range(1, 50, 1)):  # df_train.values.shape[1]
    ...
    for split_nb, (tr_ids, ts_ids) in enumerate(kfold.split(ids, censored)):
        ...
        if selectors[split_nb] is None:
            # The selector is built and fitted to the data,
            # but does not select the variables yet at this point
            ...
            X_train_selected = selector.fit(X_train_local.values,
                                            Y_train_local)
        ...
        for i in range(n_repeats):
            ...
            ci_train = concordance_index_censored(Y_train_local["event"],
                                                  Y_train_local["time"], train_pred)
            ci_test = concordance_index_censored(Y_test_local["event"],
                                                 Y_test_local["time"], test_pred)
